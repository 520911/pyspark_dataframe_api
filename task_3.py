...
df.filter((df['date'].between('2021-03-25', '2021-03-31')) & (df['location'].startswith('Russ'))).\
    withColumn('previous_date_cases', F.lag('new_cases').over(w)).\
    select('date', 'previous_date_cases', 'new_cases', (F.col('new_cases') - F.col('previous_date_cases')).alias('delta')).show(10)
